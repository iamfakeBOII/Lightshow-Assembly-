 ...
 * CSC Fall 2022
 * 
 * Student name: Rayyan Jamadar
 * Student UVic ID: V00999220
 * Date of completed work: 10 / 12 / 2023
 *
 *
...
void led_state(uint8_t LED, uint8_t state) {
	...
	switch (LED) {
		case 0: // State check in every case
		if (state > 0) { 
			PORTL |= 0b10000000; 
			break;
		}
		PORTL &= ~(0b10000000); // Negate and Update
		break;

		case 1:
		if (state > 0) { 
			PORTL |= 0b00100000; // Bitwise OR with original value
			break;
		}
		PORTL &= ~(0b00100000); 
		break;
...
void SOS() {
	...
	int length = 19; // Loop bound
	
	for (int i = 0; i < length; i++) {

		int delay = duration[i]; // Define delay for each iteration

		if (light[i] == 0xF) { 
			PORTL |= 0b10101010; // Change PORTL value
		}
			...
			PORTL = 0b00000000; // Reset case
		}
		
		_delay_ms(delay); // Call delay with loop variab
	}
}
...
void glow(uint8_t LED, float brightness) {
	...
	int threshold = brightness* PWM_PERIOD; 
...
void pulse_glow(uint8_t LED) {
	...
	for (;;) { // Infinite loop
		...
		if (trigger <= 0 && slow_count > 5) {  
			threshold--;
			slow_count = 0;
		}

		if (trigger >= 1 && slow_count > 5) { 
			threshold++;
			slow_count = 0;
		}
